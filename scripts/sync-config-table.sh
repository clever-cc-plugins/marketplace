...
if [[ -d "$ROOT/docs" ]]; then
  while IFS= read -r -d '' f; do
    relpath="${f#$ROOT/}"
    config_files+=("$relpath")
  done < <(find "$ROOT/docs" -maxdepth 1 -type f -name '*.md' -print0 2>/dev/null | sort -z)
fi
...
while IFS= read -r line; do
  ...
  if $section_found; then
    if [[ "$line" =~ ^\|[[:space:]]*\`([^\`]+)\`[[:space:]]*\|[[:space:]]*(.+)[[:space:]]*\| ]]; then
      file="${BASH_REMATCH[1]}"
      desc="${BASH_REMATCH[2]}"
      # Trim trailing padding the greedy regex captures before the closing pipe
...
# Normalize with prettier before diffing so formatting-only churn doesn't trigger a rewrite
